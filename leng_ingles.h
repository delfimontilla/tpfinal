...
#define MSJ_BIENVENIDA "Welcome to Simpletron! Please enter your program one instruction (or data word) at a time. I will type the location number and a question mark (?). You then type the word for that location. Verify that the opcode 45 has been entered for the correct execution of the program. Type the sentinel -99999999 to stop entering your program."
#define MSJ_CARGA_COMPLETA "*** Program upload complete ***"
#define MSJ_COMIENZO_EJECUCION "*** Program execution starts ***"

#define MSJ_ACLARACION_AYUDA1 "The arguments are positional; they may be present or omited (in this case they'll have their default value), that's why they should be written in the order shown next."
#define MSJ_ACLARACION_AYUDA2 "The first argument posible is '-m', which will preceed the number of words to enter. The next one if the dump format, entering '-f' followed by txt or bin (the default option is txt). If you want the entry of information by stdin, you should write '-', otherwise you have to write the names of the files with the format before, for example t:suma.lms for a txt file or b:suma.lms for a bin file, in case of not specifying the format it'll be taken as a txt"
